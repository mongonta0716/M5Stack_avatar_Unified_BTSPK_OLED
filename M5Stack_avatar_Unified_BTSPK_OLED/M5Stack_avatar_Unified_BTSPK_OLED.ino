...
//  }
  ...
  if (oled.width() > 160)
  {
    oled.setFont(&fonts::DejaVu12);
  }
  oled.setFont(&fonts::lgfxJapanGothic_12);
  oled.setEpdMode(epd_mode_t::epd_fastest);
  oled.setCursor(0, 12);
  oled.startWrite();
  oled.print("BT A2DP : ");
  oled.println(bt_device_name);
...
void loop(void)
{
  ...
  if (bits)
  {
    ...
    for (int id = 0; id < 8; ++id)
    {
      ...
      oled.setCursor(0, 12 + id * 12);
      oled.fillRect(0, 12 + id * 12, oled.width(), 12, oled.getBaseColor());
      oled.print(a2dp_sink.getMetaData(id));
    }
    oled.endWrite();
  }
  ...
  { // draw volume bar
    ...
    int x = v * (oled.width()) >> 8;
    if (px != x)
    {
      oled.fillRect(x, 8, px - x, 3, px < x ? TFT_GREEN : TFT_BLACK);
      oled.display();
      px = x;
    }
  }

  if (fft_enabled && !oled.displayBusy())
  { // draw stereo level meter
    ...
    if (data)
    {
      ...
      for (int i = 0; i < 2; ++i)
      {
        int x = (level[i] * oled.width() - 4) / INT16_MAX;
        int px = prev_x[i];
        if (px != x)
        {
          oled.fillRect(x, i * 4, px - x, 3, px < x ? 0x0055FFu : 0u);
          prev_x[i] = x;
        }
        px = peak_x[i] - 1;
        if (px > x)
        {
          oled.writeFastVLine(px + 1, i * 4, 3, TFT_BLACK);
        }
        else
        {
          px = x + 1;
        ...
        {
          peak_x[i] = px;
          oled.writeFastVLine(px, i * 4, 3, TFT_WHITE);
        }
      }
      oled.display();

      int dsp_height = oled.height();
      ...
      for (int x = 0; x < xe; ++x)
      {
        ...
        if (y != py)
        {
          oled.fillRect(x*4, y, 3, py - y, (y < py) ? 0x0099FFu : 0u);
          prev_y[x] = y;
        }
        py = peak_y[x] + 1;
        if (py < y)
        {
          oled.writeFastHLine(x*4, py-1, 3, TFT_BLACK);
        }
        else
        {
          py = y - 1;
